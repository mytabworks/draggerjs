...
const fileProxy = async (options, file) => {
	...
	const proxyPkg = {
		name: `${pkgName}/${file}`,
		private: true,
		main: path.join('..', cjsDir, `${file}`),
		module: path.join('..', esmDir, `${file}`),
	}

	if (typeof typesDir === 'string') {
		proxyPkg.types = path.join('..', typesDir, `${file}`)
	} else if (await isFile(path.join(cwd, `${file}`))) {
		proxyPkg.types = path.join('..', `${file}`)
		// try the esm path in case types are located with each
	} else if (await isFile(path.join(cwd, esmDir, `${file}`))) {
		proxyPkg.types = path.join('..', esmDir, `${file}`)
	}
